...
public:
	...
	string rejestracja() {
		...
		cout << "  Przeciwnik ma wystarczająco wojsk na 3 ataki, jeśli odeprzesz wszystkie 3, wygrasz wojnę. Powodzenia." << endl << endl;
		_getch();
		...
		cout << "  a z kolei poziom koszar pozwoli ci na rekrutację lepszą opłacalność rekrutajci wojsk." << endl << endl;
		_getch();
		...
		cout << "  informacje na temat postaci i ilości zrekrutowanych wojsk czy ilości pozostałego czasu do starcia. " << endl << endl;
		_getch();
		...
		cout << "  Naciśnij dowlny klawisz, by rozpocząć grę" << endl;
		_getch();
		system("cls");
		return imie;
	}
...
public:
	...
	int* rekrutacja() {
		...
		cout << endl;
		cout << "  Za 1 drewna , 1 kamienia , 1 metalu możesz zrekrutować " << poziom << " jednostek" << endl;
		cout << "  Wpisz ile wojowników chcesz zrekrutować: " << endl << endl;
		cout << "  Otrzymasz : " << poziom << " x" << " ";


		cin >> rekruci;
		rekruci = rekruci * poziom;
		system("cls");
		if (rekruci <= Gracz::drewno && rekruci <= Gracz::kamien && rekruci <= Gracz::metal) {
			if (rekruci != 1)
			{
				cout << endl;
			...
			Gracz::drewno -= rekruci;
			Gracz::kamien -= rekruci;
			Gracz::metal -= rekruci;
...
public:
	...
	int atakuj() {
		cout << endl;
		cout << "  Nadszedł czas, " << numer_ataku << " bitwa z naszym wrogiem" << endl;
		cout << "  My mamy " << Gracz::liczba_wojsk << " wojsk" << endl;
		cout << "  A ich armia liczy " << wojska << " wojowników" << endl << endl;
		cout << "  Naciśnij dowolny klawisz by zobaczyć wynik bitwy" << endl << endl;
		_getch();
		if (Gracz::liczba_wojsk >= wojska) {
		...
		if (Gracz::liczba_wojsk >= wojska && numer_ataku == 3) {
...
int main()
{
	...
	while (koniec_gry)
	{
		do {
			...
			case '1':
				...
				case '5':
					system("cls");
					cout << endl;
					cout << "                                           WASZA KTÓRLEWSKA MOŚĆ                                           " << endl;
					cout << "                                                                                                           " << endl;
					cout << "                                                                                                           " << endl;
		...
		if (gracz.czas_do_bitwy <= 0) {
			gracz.liczba_wojsk -= przeciwnik.atakuj() - 1500;
